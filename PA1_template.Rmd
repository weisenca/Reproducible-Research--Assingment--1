...
The mean number of steps taken per day is `r working.mean`
...
The median number of steps taken per day is `r working.med `

```{r,echo=FALSE}
## Clear variables
remove(dates,temp,i,total)
```
...
The 5-minute interval with the most steps was interval `r which(total == max(total))`

```{r,echo=FALSE}
## Clear variables
remove(inter,temp,i,total)
```
...
The data set contained `r missval` missing data values.
...
The mean number of steps taken per day using the new data set is `r new.mean`
...
The median number of steps taken per day using the new data set is `r as.integer(new.med)`
...
```{r,echo=FALSE}
## Remove variables
remove(dates,missing.data,replacement.data,temp,i,total)
```
